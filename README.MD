...
### Building the App
A. On localhost
- Clone the repository git clone <repo-url> and head to the root directory of the folder
- cd /catch-n-replace/

- Install Nodemon or run node app.js on the terminal and head to the browser http://localhost:3211

B. On Container using Docker.

- Build an Docker image with the following command
...
- Install Apache server using "https://ubuntu.com/tutorials/install-and-configure-apache#1-overview" 
- Build the app with above the mentioned steps.
...
```git clone https://github.com/restlessankyyy/catch-n-replace.git && cd /$current_dir/catch-n-replace ```
...
``` gcloud container clusters create catch-n-replace --num-nodes 2 --machine-type n1-standard-1 --zone us-central1-a ```
...
Creating cluster catch-n-replace done.
<br/>
...
``` kubectl create deployment catch-n-replace  --image=gcr.io/PROJECT_ID/catch-n-replace:v1 ```
